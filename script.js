// ========================================
// CONFIGURAÇÕES - BASE SEPOLIA TESTNET
// ========================================
const CONFIG = {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ SEM auto-conexão - usuário clica para conectar
...
// ========================================
// CONEXÃO WALLET (REESCRITA DO ZERO)
// ========================================
async function connectWallet() {
  console.log('🔌 Iniciando conexão...');
  
  // Verifica se tem MetaMask
  if (!window.ethereum) {
  ...
  try {
    ...
    if (chainId !== CONFIG.network.chainIdDecimal) {
      ...
      // Após trocar rede, recarrega a página
      return;
    }
...
async function buyTokens() {
  ...
  try {
    ...
    const allowance = await tokenContract.methods.allowance(userAccount, CONFIG.presaleContract).call();
    ...
    // Estima gas
    const gasEstimate = await presaleContract.methods.buyTokens(amountWei).estimateGas({ from: userAccount });
...
function handleAccountsChanged(accounts) {
  if (accounts.length === 0) {
    disconnectWallet();
  } else if (accounts[0].toLowerCase() !== userAccount) {
    userAccount = accounts[0].toLowerCase();
    web3 = new Web3(window.ethereum);
    onWalletConnected();
  }
...
async function claimTokens() {
  ...
  try {
    const presaleContract = new web3.eth.Contract(PRESALE_ABI, CONFIG.presaleContract);
    const gasEstimate = await presaleContract.methods.claimTokens().estimateGas({ from: userAccount });
...
async function updateClaimableAmount() {
  ...
  try {
    const presaleContract = new web3.eth.Contract(PRESALE_ABI, CONFIG.presaleContract);
    const claimable = await presaleContract.methods.claimableAmount(userAccount).call();
