...
async function initializeWeb3(address) {
  try {
    ...
    if (chainId !== CONFIG.network.chainIdDecimal) {
      console.log('⚠️ Rede incorreta. Solicitando mudança para Base Sepolia...');
      showNotification('Please switch to Base Sepolia Testnet', 'info');
...
async function checkApproval() {
  ...
  try {
    ...
    const amount = web3.utils.toWei(elements.payAmount.value, 'mwei');
